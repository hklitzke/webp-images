...
{
    private static $webp_default_quality = 85;
    ...
    public function createWebPImage($path, $filename, $hash, $variant = false)
    {
        if (function_exists('imagewebp') && function_exists('imagecreatefromjpeg') && function_exists('imagecreatefrompng')) {
			...
			$basename = $_SERVER['DOCUMENT_ROOT'].'/public/assets/' . $filename;
            list($width, $height, $type, $attr) = getimagesize($path);

            switch ($type) {
                case 2:
                    $img = imagecreatefromjpeg($path);
                    imagewebp($img, $this->createWebPName($orgpath), 85);
/*
                    $img = imagecreatefromjpeg($basename);
                    imagewebp($img, $this->createWebPName($basename), $this->webp_default_quality);
*/
                    imagedestroy($img);
                    break;
                case 3:
                    ...
                    imagewebp($img, $this->createWebPName($orgpath), 85);
/*
					$img = imagecreatefrompng($basename);
					imagepalettetotruecolor($img);
...
*/
    ...
    public function delete ($filename, $hash)
    {
        ...
        foreach (glob($_SERVER['DOCUMENT_ROOT'] . "/public/assets/" . $basename . "__*.webp") as $delfile) {
		    unlink($delfile);
		}
		if(file_exists($_SERVER['DOCUMENT_ROOT'] . "/public/assets/" . $filename . ".webp")){
			unlink($_SERVER['DOCUMENT_ROOT'] . "/public/assets/" . $filename . ".webp");
		}
